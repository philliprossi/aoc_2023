    ...
    def get_locations(beams, columns_len, rows_len):
        ...
        for beam in beams:
            for location in beam.locations:
                if not (location[0] < 0 or location[0] >= columns_len or location[1] < 0 or location[1] >= rows_len):
                    locations_set.add(location)
        return locations_set

    def __hash__(self):
        return hash((self.x, self.y, self.direction, self.is_moving))  # Replace attr1, attr2 with the attributes that uniquely identify a Beam object
    ...
    while Beam.any_beams_moving(beams):
        for beam in beams:
            ...
            if beam.get_coord() in mirrors:
                beam.set_new_direction(mirrors[beam.get_coord()].get_new_direction(beam.get_direction()))
